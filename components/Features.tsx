    ...
    <Section className="border-t border-white/10 bg-black">
      <div className="grid items-start gap-6 md:grid-cols-6 lg:grid-cols-12 lg:gap-10">
        ...
        <Card className="overflow-hidden md:col-span-3 lg:col-span-6">
          {experience?.image ? (
            <>
              <img src={getMediaUrl(experience.image)} alt="Interior of London Karaoke Club" className="hidden h-[420px] w-full object-cover md:block" loading="lazy" />
              <img src={getMediaUrl(experience.mobileImage || experience.image)} alt="Interior of London Karaoke Club" className="block h-[320px] w-full object-cover md:hidden" loading="lazy" />
            </>
          ) : (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {(grid?.items || []).map((item, idx) => (
            <Card key={idx} className="overflow-hidden">
              {item.image ? (
                <img src={getMediaUrl(item.image)} alt={item.title} className="h-44 w-full object-cover" loading="lazy" />
              ) : (
