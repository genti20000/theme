...
const Hero: React.FC = () => {
  ...
  const mobileSlides = heroData.mobileSlides && heroData.mobileSlides.length > 0 ? heroData.mobileSlides : slides;
  ...
  // Create snowflakes with unique IDs and positions
    ...
    <section className="relative h-[90vh] md:h-screen flex flex-col items-center justify-end pb-12 md:pb-24 text-center text-white overflow-hidden bg-black" aria-label="Welcome">
      ...
      >
        {slides.map((slide, index) => {
             ...
             const mobileSlide = mobileSlides[index] || slide;
             return (
                <div 
                  key={index} 
                ...
                >
                    ...
                    <div className="hidden md:block w-full h-full">
                        {isVideo(slide) ? (
                            ...
                            <img 
                              src={slide} 
                              alt={index === 0 ? "London Karaoke Club main background" : `Background slide ${index + 1}`}
        ...
        {showButtons && (
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
              <a 
                ...
                className="bg-red-600 hover:bg-red-700 text-white text-lg font-bold py-3 px-10 rounded-full border-2 border-white transition-all transform hover:scale-105 shadow-[0_0_30px_rgba(220,38,38,0.6)]"
              >
                {heroData.buttonText}
              </a>
              <button 
                onClick={() => window.scrollTo({top: window.innerHeight, behavior: 'smooth'})} 
                className="bg-white/10 backdrop-blur-md hover:bg-white/20 text-white font-bold py-3 px-10 rounded-full border border-white/20 transition-all"
