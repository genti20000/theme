...
import { HeaderData, HeroData, HighlightsData, FeaturesData, VibeData, BatteryData, GalleryData, BlogData, FAQData, DrinksData, FirebaseConfig, TermItem, Song } from '../types';
...
const Input: React.FC<{ label: string; value: string; onChange: (v: string) => void; type?: string }> = ({ label, value, onChange, type = 'text' }) => (
  <div className="mb-6">
    ...
    <input 
      type={type} 
      className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-white outline-none focus:border-pink-500 transition-all font-medium text-sm" 
      value={value || ''} 
...
const CodeArea: React.FC<{ label: string; value: string; onChange: (v: string) => void }> = ({ label, value, onChange }) => (
    <div className="mb-6">
      ...
      <textarea 
        rows={6} 
        className="w-full bg-black border border-zinc-700 rounded-xl px-4 py-3 text-green-500 font-mono outline-none focus:border-pink-500 transition-all text-xs leading-tight" 
        value={value || ''} 
        onChange={(e) => onChange(e.target.value)} 
      />
    </div>
...
const AdminDashboard: React.FC = () => {
  ...
  const { 
    ...
    featuresData, updateFeaturesData, vibeData, updateVibeData, updateFoodMenu,
    drinksData, updateDrinksData, updateTestimonialsData,
    infoSectionData, updateInfoSectionData, updateEventsData,
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 space-y-2">
            <input 
              className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2 text-white outline-none focus:border-pink-500 transition-all font-medium text-xs" 
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-[10px] font-black uppercase py-2.5 rounded-xl border border-zinc-700 transition-all"
              >
                Upload Local
              </button>
              <button 
...
?>`;
  ...
  const authenticate = () => {
    // In a real application, this would be a secure API call to verify the password
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-6 text-white">
      <div className="bg-zinc-900 p-12 rounded-[3rem] border border-zinc-800 w-full max-w-md shadow-2xl">
        ...
        <input 
          ...
          className="w-full bg-zinc-800 border border-zinc-700 p-5 rounded-2xl outline-none focus:border-pink-500 text-white mb-6 text-center text-xl tracking-[0.5em]" 
          placeholder="••••••••" 
        ...
        <button 
          ...
          className="w-full bg-pink-600 text-white font-black py-5 rounded-2xl hover:bg-pink-500 transition-all uppercase tracking-widest text-sm shadow-lg active:scale-95"
        >
          Enter Control Room
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white pb-20 font-sans">
      ...
      <div className="bg-zinc-900/80 backdrop-blur-xl border-b border-zinc-800 sticky top-0 z-50 p-6 flex justify-between items-center px-10">
        ...
        <div className="flex gap-4">
            <button 
              ...
              className="bg-pink-600 hover:bg-pink-500 px-10 py-3 rounded-xl text-[12px] font-black uppercase tracking-widest animate-pulse shadow-xl"
            >
      ...
      <div className="flex bg-zinc-900/50 border-b border-zinc-800 overflow-x-auto scrollbar-hide px-8 sticky top-[88px] z-40 backdrop-blur-md">
        {['seo', 'nav', 'hero', 'about', 'features', 'vibe', 'stats', 'drinks', 'events', 'blog', 'faq', 'info', 'gallery', 'terms', 'config'].map(t => (
            <button 
              key={t} 
              onClick={() => setTab(t)} 
              className={`px-4 py-5 uppercase font-black text-[10px] tracking-widest transition-all relative flex-shrink-0 ${tab === t ? 'text-pink-500' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
                {t}
      ...
      <div className="container mx-auto p-10 max-w-5xl">
                ...
                <div className="space-y-4">
                    ...
                    {heroData.slides.map((s, i) => (
                        <div key={i} className="bg-zinc-800/20 p-4 rounded-2xl border border-zinc-800">
                            ...
                            <button 
                              onClick={() => updateHeroData(prev => ({...prev, slides: prev.slides.filter((_, idx) => idx !== i), mobileSlides: prev.mobileSlides?.filter((_, idx) => idx !== i)}))} 
                    ...
                    <button 
                      onClick={() => updateHeroData(prev => ({...prev, slides: [...prev.slides, ''], mobileSlides: [...(prev.mobileSlides || []), '']}))} 
                      className="w-full py-4 border-2 border-dashed border-zinc-800 text-xs text-zinc-500 font-black rounded-2xl hover:border-pink-500 hover:text-pink-500 transition-all"
                    >
                      + ADD NEW SLIDE
                    </button>
                </div>
        ...
        {tab === 'about' && (
            <SectionCard 
              title="The Soho Highlights" 
              enabled={highlightsData.enabled} 
              onToggle={v => updateHighlightsData(prev => ({...prev, enabled: v}))}
            >
                ...
                <div className="space-y-4">
                    ...
                    {highlightsData.featureList.map((f, i) => (
                        <div key={i} className="flex gap-4">
                            <input 
                              className="flex-1 bg-zinc-800 border border-zinc-700 p-3 rounded-xl text-xs text-white" 
                              value={f} 
                              onChange={e => {
                            ...
                            />
                            <button 
                              onClick={() => updateHighlightsData(prev => ({...prev, featureList: prev.featureList.filter((_, idx) => idx !== i)}))} 
                              className="bg-red-900/20 text-red-500 px-4 rounded-xl font-black"
                            >
                              ×
                            </button>
                        </div>
                    ...
                    <button 
                      onClick={() => updateHighlightsData(prev => ({...prev, featureList: [...prev.featureList, '']}))} 
                      className="w-full py-2 bg-zinc-800 text-zinc-500 font-black rounded-xl text-[10px]"
                    >
                      + ADD FEATURE
                    </button>
                </div>
        ...
        {tab === 'features' && (
            <div className="space-y-12">
                <SectionCard 
                  ...
                  onToggle={v => updateFeaturesData(prev => ({...prev, enabled: v}))}
                >
        ...
        {tab === 'vibe' && (
            <SectionCard 
              title="The Vibe" 
              enabled={vibeData.enabled} 
              onToggle={v => updateVibeData(prev => ({...prev, enabled: v}))}
            >
        ...
        {tab === 'stats' && (
            <SectionCard 
              title="LKC Statistics" 
              enabled={batteryData.enabled} 
              onToggle={v => updateBatteryData(prev => ({...prev, enabled: v}))}
            >
        ...
        {tab === 'faq' && (
            <SectionCard 
              ...
              enabled={faqData.enabled} 
              onToggle={v => updateFaqData(prev => ({...prev, enabled: v}))}
            >
        ...
        {tab === 'gallery' && (
            ...
            <SectionCard title="Gallery Options">
                <div className="flex items-center gap-4">
                    <input 
                      type="checkbox" 
                      checked={galleryData.showOnHome} 
                      onChange={e => updateGalleryData(prev => ({...prev, showOnHome: e.target.checked}))} 
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-10">
                    {galleryData.images.map((img, i) => (
                        <div key={img.id} className="relative group aspect-square rounded-2xl overflow-hidden border border-zinc-800 shadow-xl">
                            ...
                            <button 
                              onClick={() => updateGalleryData(prev => ({...prev, images: prev.images.filter((_, idx) => idx !== i)}))} 
                    ...
                    <div className="aspect-square border-2 border-dashed border-zinc-800 rounded-2xl flex flex-col items-center justify-center bg-zinc-900/50 hover:border-pink-500 group transition-all">
                        ...
                        <button 
                          ...
                          className="text-[11px] font-black uppercase text-zinc-500 group-hover:text-pink-500 transition-colors"
                        >
                          Batch Upload
                        </button>
                    </div>
            ...
            <div className="space-y-12">
                <SectionCard title="Custom Scripts (Head/Body)">
                    <CodeArea 
                      ...
                      onChange={v => updateHeaderData(prev => ({...prev, customScripts: {...prev.customScripts, header: v}}))} 
                    />
                    <CodeArea 
                      label="Footer Scripts (Before </body>)" 
                      value={headerData.customScripts?.footer || ''} 
                      onChange={v => updateHeaderData(prev => ({...prev, customScripts: {...prev.customScripts, footer: v}}))} 
                    />
                </SectionCard>
                ...
                <SectionCard title="Server PHP Snippet">
                    ...
                    <div className="bg-black p-8 rounded-[2rem] border border-zinc-800 overflow-x-auto relative mb-6">
                        ...
                        <button 
                          onClick={() => { navigator.clipboard.writeText(phpSnippet); alert("Copied!"); }} 
                          className="absolute top-6 right-6 bg-zinc-800 hover:bg-zinc-700 px-5 py-2 rounded-xl text-[10px] font-black"
                        >
                          COPY PHP
                        </button>
                    </div>
                ...
                <button 
                  onClick={purgeCache} 
                  className="w-full py-5 bg-red-600/10 border border-red-600 text-red-500 rounded-2xl font-black uppercase tracking-widest"
                >
                  Reset Local Cache
                </button>
            </div>
