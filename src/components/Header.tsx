...
const Header: React.FC<HeaderProps> = ({ currentPage, onNavigate }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-black shadow-2xl border-b border-zinc-900" role="banner">
      <style>{`
      ...
      <div className="container mx-auto px-4 py-4 grid grid-cols-[1fr_auto_1fr] items-center">
        <div className="flex justify-start items-center">
          <a 
            ...
            aria-label="Book Now - External link to booking page"
          >
        ...
        <div className="flex justify-center items-center">
          <button 
            ...
            className="focus:outline-none z-10 transition-transform duration-300 hover:scale-105"
      ...
      {isMenuOpen && (
        <div className="fixed top-[80px] left-0 w-full h-[calc(100vh-80px)] pointer-events-none overflow-hidden z-40" role="dialog" aria-modal="true" aria-label="Navigation menu">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm pointer-events-auto transition-opacity duration-500" onClick={() => setIsMenuOpen(false)} aria-hidden="true"></div>
          <div className="relative w-full px-4 pt-8 flex justify-between gap-4 pointer-events-none">
            <div className="w-1/2 bg-gradient-to-br from-zinc-900 to-purple-900 border-2 border-white rounded-tl-2xl rounded-tr-sm rounded-bl-[60px] rounded-br-3xl p-6 shadow-[0_0_30px_rgba(147,51,234,0.6)] flex flex-col items-center gap-6 animate-wing-left origin-top-right pointer-events-auto">
              {leftLinks.map((link, idx) => (
                <button 
                  ...
                  className={`text-xl md:text-3xl font-black tracking-wider transition-colors animate-item-slide-up opacity-0 fill-mode-forwards ${isActivePage(link) ? 'text-pink-400' : 'text-gray-100 hover:text-pink-400'}`}
                >
                  {getLabel(link)}
                </button>
              ))}
            ...
            <div className="w-1/2 bg-gradient-to-bl from-zinc-900 to-purple-900 border-2 border-white rounded-tr-2xl rounded-tl-sm rounded-br-[60px] rounded-bl-3xl p-6 shadow-[0_0_30px_rgba(147,51,234,0.6)] flex flex-col items-center gap-6 animate-wing-right origin-top-left pointer-events-auto">
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                className="text-xl md:text-3xl font-black tracking-wider text-yellow-400 hover:text-yellow-300 transition-colors animate-item-slide-up opacity-0 fill-mode-forwards" 
                style={{animationDelay: `${0.3 + rightLinks.length*0.1}s`}}
              >
                BOOK NOW
              </a>
            </div>
